...
def create_room_table(conn: sqlite3.Connection, node: Node):
    """create a table from the create_table_sql statement
    :param conn: Connection object
    :param create_table_sql: a CREATE TABLE statement
    :return:
...
def create_room(conn: sqlite3.Connection, new_room: Room) -> int:
    """
    Create a new project into the projects table
    :param conn:
    :param object:
    ...
    """
    name = new_room.name
    x = new_room.location[0]
    y = new_room.location[1]
    sql = """ INSERT INTO rooms(name,x,y,timestamp)
              VALUES(?,?,?) """
    cur = conn.cursor()
    cur.execute(sql, (name, x, y))
    conn.commit()
    return cur.lastrowid
